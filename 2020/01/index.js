...
const looper = (index, lap, factor) => {
    for (index; index < (input.length - factor + lap); index++) {
        values[lap] = parseInt(input[index]);
        if (lap === factor) {
            const sumOfValues = values.reduce((a, b) => a + b, 0);
            if (sumOfValues === 2020) {
                console.log('COOL BEANS! - values to sum up to 2020', values);
                console.log('\n\t', values.reduce((a, b) => a * b, 1));
                return;
            }
        }
        if (lap < factor) {
            looper(index+1, lap+1, factor);
        }
    }
}
